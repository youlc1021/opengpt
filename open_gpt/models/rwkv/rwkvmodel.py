...
class RWKV(MyModule):
    def __init__(self, model, strategy, verbose=False, convert_and_save_and_exit=None):
        super().__init__()
        if verbose:
        ...
        with torch.no_grad():
            self.w = model.state_dict()  # load model to CPU first
            gc.collect()
            w = self.w
            ...
            args.n_embd = w['rwkv.embeddings.weight'].shape[1]
            args.n_layer = 0
            keys = list(w.keys())
            for x in keys:
                layer_id = int(x.split('.')[2]) if ('rwkv.blocks.' in x) else 0
            ...
            if not ALREADY_CONVERTED:
                try:  # precompute embedding
                    w['rwkv.embeddings.weight'] = F.layer_norm(w['rwkv.embeddings.weight'], (args.n_embd,),
                                                               weight=w['rwkv.blocks.0.pre_ln.weight'],
                                                               bias=w['rwkv.blocks.0.pre_ln.bias'])
                except:
                    w['rwkv.embeddings.weight'] = F.layer_norm(w['rwkv.embeddings.weight'].float(), (args.n_embd,),
                                                   weight=w['rwkv.blocks.0.pre_ln.weight'].float(),
                                                   bias=w['rwkv.blocks.0.pre_ln.bias'].float())
                del w['rwkv.blocks.0.pre_ln.weight']
                del w['rwkv.blocks.0.pre_ln.bias']
            ...
            for x in keys:
                ...
                layer_id = int(x.split('.')[2]) if ('rwkv.blocks.' in x) else 0
                if ('rwkv.ln_out.' in x) or ('head.' in x):
                ...
                if not ALREADY_CONVERTED:
                    if self.RESCALE_LAYER > 0:
                        if 'rwkv.attention.output.weight' in x:
                            w[x] = w[x] / (2 ** int(layer_id // self.RESCALE_LAYER))
                        if 'rwkv.feed_forward.value.weight' in x:
                            w[x] = w[x] / (2 ** int(layer_id // self.RESCALE_LAYER))
                    ...
                    if '.time_decay' in x:  # need fp32 for this
                        ...
                        if (len(w[x].shape) == 2) and ('embeddings' not in x):
                ...
                if 'feed_forward.value.weight' in x:
                    gc.collect()
    ...
    @MyFunction
    def ffn_one(self, x, sx, ln_w, ln_b, k_mix, r_mix, kw, vw, rw, kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx,
                rmy, rry):
    ...
    @MyFunction
    def ffn_seq(self, x, sx, ln_w, ln_b, k_mix, r_mix, kw, vw, rw, kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx,
                rmy, rry):
        xx = F.layer_norm(x, (x.shape[-1],), weight=ln_w, bias=ln_b)
    ...
    @MyFunction
    def att_one(self, x, sx, aa, bb, pp, ln_w, ln_b, k_mix, v_mix, r_mix, t_decay, t_first, kw, vw, rw, ow, kmx, krx,
                kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry, omx, orx, omy, ory):
        xx = F.layer_norm(x, (x.shape[-1],), weight=ln_w, bias=ln_b)
        kx = xx * k_mix + sx * (1 - k_mix)
    ...
    @MyFunction
    def att_seq(self, x, sx, aa, bb, pp, ln_w, ln_b, k_mix, v_mix, r_mix, t_decay, t_first, kw, vw, rw, ow, kmx, krx,
                kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry, omx, orx, omy, ory):
        xx = F.layer_norm(x, (x.shape[-1],), weight=ln_w, bias=ln_b)
        sx = torch.cat((sx.unsqueeze(0), xx[:-1, :]))
        kx = xx * k_mix + sx * (1 - k_mix)
    ...
    @MyFunction
    def att_seq_i8(self, x, sx, aa, bb, pp, ln_w, ln_b, k_mix, v_mix, r_mix, t_decay, t_first, kw, vw, rw, ow, kmx, krx,
                   kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry, omx, orx, omy, ory):
        xx = F.layer_norm(x, (x.shape[-1],), weight=ln_w, bias=ln_b)
        sx = torch.cat((sx.unsqueeze(0), xx[:-1, :]))
        kx = xx * k_mix + sx * (1 - k_mix)
    ...
    if os.environ["RWKV_CUDA_ON"] == '1':
        ...
        def cuda_att_seq(self, x, sx, aa, bb, pp, ln_w, ln_b, k_mix, v_mix, r_mix, t_decay, t_first, kw, vw, rw, ow,
                         kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry, omx, orx, omy, ory):
            T, C = x.size()
        ...
        @MyFunction
        def cuda_att_seq_i8(self, x, sx, aa, bb, pp, ln_w, ln_b, k_mix, v_mix, r_mix, t_decay, t_first, kw, vw, rw, ow,
                            kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry, omx, orx, omy, ory):
            T, C = x.size()
            xx = F.layer_norm(x, (C,), weight=ln_w, bias=ln_b)
            sx = torch.cat((sx.unsqueeze(0), xx[:-1, :]))
        ...
        with torch.no_grad():
            ...
            if state == None:
                ...
                for i in range(args.n_layer):  # state: 0=att_xx 1=att_aa 2=att_bb 3=att_pp 4=ffn_xx
                    ...
                    dev = dd.device
                    atype = dd.atype
                    state[i * 5 + 0] = torch.zeros(args.n_embd, dtype=atype, requires_grad=False,
                                                   device=dev).contiguous()
                    state[i * 5 + 1] = torch.zeros(args.n_embd, dtype=torch.float, requires_grad=False,
                                                   device=dev).contiguous()
                    state[i * 5 + 2] = torch.zeros(args.n_embd, dtype=torch.float, requires_grad=False,
                                                   device=dev).contiguous()
                    state[i * 5 + 3] = torch.zeros(args.n_embd, dtype=torch.float, requires_grad=False,
                                                   device=dev).contiguous() - 1e30
                    state[i * 5 + 4] = torch.zeros(args.n_embd, dtype=atype, requires_grad=False,
                                                   device=dev).contiguous()

            seq_mode = len(tokens) > 1

            x = w['rwkv.embeddings.weight'][tokens if seq_mode else tokens[0]]

            for i in range(args.n_layer):
                bbb = f'rwkv.blocks.{i}.'
                att = f'rwkv.blocks.{i}.attention.'
                ffn = f'rwkv.blocks.{i}.feed_forward.'
                dd = self.strategy[i]
                dev = dd.device
                atype = dd.atype
                wtype = dd.wtype
                ...
                x, state[i * 5 + 0], state[i * 5 + 1], state[i * 5 + 2], state[i * 5 + 3] = ATT(
                    ...
                    w[f'{att}time_mix_key'], w[f'{att}time_mix_value'], w[f'{att}time_mix_receptance'],
                ...
                x, state[i * 5 + 4] = FFN(
                    ...
                    w[f'{ffn}time_mix_key'], w[f'{ffn}time_mix_receptance'],
                    kw, vw, rw,
                    kmx, krx, kmy, kry,
                    vmx, vrx, vmy, vry,
                    rmx, rrx, rmy, rry,
            ...
            x = F.layer_norm(x, (args.n_embd,), weight=w['rwkv.ln_out.weight'], bias=w['rwkv.ln_out.bias'])
            if w['head.weight'].dtype != torch.uint8:
                x = x @ w['head.weight']
            else:
                if seq_mode and full_output:
                    x = self.mm8_seq(x, w['head.weight'], w['head.weight_mx'], w['head.weight_rx'], w['head.weight_my'],
                                     w['head.weight_ry'])
                else:
                    x = self.mm8_one(x, w['head.weight'], w['head.weight_mx'], w['head.weight_rx'], w['head.weight_my'],
                                     w['head.weight_ry'])
